...
class AudioGattServerManager(
    ...
    fun openGattServer() {
        ...
        try {
            val mgr = activity.getSystemService(Activity.BLUETOOTH_SERVICE) as BluetoothManager
            server = mgr.openGattServer(activity, gattCb)?.apply {
    ...
    private val gattCb = object : BluetoothGattServerCallback() {
        override fun onConnectionStateChange(
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                clients[address] = dev
                retries[address] = 0 // Initialize retry count for new connection
                subs[address] = false // Initialize subscription state
