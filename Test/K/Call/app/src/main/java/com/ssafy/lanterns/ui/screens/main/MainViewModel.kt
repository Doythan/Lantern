...
class MainViewModel @Inject constructor(
    ...
    private val advertiseRunnable: Runnable = Runnable { startOrUpdateAdvertising(isEmergency = false) }
    ...
    private fun startBleOperationsInternal() {
        ...
        startOrUpdateAdvertising(isEmergency = false) // 즉시 광고 및 주기적 업데이트 예약
    ...
    private fun startOrUpdateAdvertising(isEmergency: Boolean = false) {
    ...
    fun triggerEmergencyBroadcast() {
        ...
        viewModelScope.launch {
            ...
            Log.d(TAG, "NeighborAdvertiser.startAdvertising 호출 (Emergency=true)")
            ...
            if (currentlyScanning && currentlyBleReady) {
                ...
                startOrUpdateAdvertising(isEmergency = false) // 일반 광고 시작 및 주기적 업데이트 재개
            } else {
