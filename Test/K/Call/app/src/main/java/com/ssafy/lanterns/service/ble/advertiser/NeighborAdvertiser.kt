...
object NeighborAdvertiser {
    ...
    fun startAdvertising(
        serverUserId: Long,
        processedNickname: String,
        currentOwnAdvertisedDepth: Int,
        isEmergency: Boolean = false,
        profileImageNumber: Int = 0,
        ...
        Log.i(TAG, "광고 시작 요청: sID=$serverUserId, Nick='$processedNickname', MyDepth=$currentOwnAdvertisedDepth, Emergency=$isEmergency, ProfileImage=$profileImageNumber, Callable=$isCallable")
        ...
        val settings = AdvertiseSettings.Builder()
            ...
            .setConnectable(true) // 연결 가능하도록 변경 (GATT 서버 접속 허용)
            .setTimeout(0) // 타임아웃 없음 (계속 광고)
            .build()

        val lanternPacket = buildLanternPacket(serverUserId, processedNickname, currentOwnAdvertisedDepth, isEmergency, profileImageNumber, isCallable)
        ...
        currentAdvertiseCallback = object : AdvertiseCallback() {
            override fun onStartSuccess(settingsInEffect: AdvertiseSettings) {
                ...
                Log.i(TAG, "광고 성공: sID=$serverUserId, Nick='$processedNickname', MyDepth=$currentOwnAdvertisedDepth, Callable=$isCallable")
            }
            ...
            override fun onStartFailure(errorCode: Int) {
                ...
                if (errorCode == ADVERTISE_FAILED_INTERNAL_ERROR || 
                    ...
                    pendingRetryRunnable = Runnable {
                        if (!isCurrentlyAdvertising) {
                            startAdvertising(serverUserId, processedNickname, currentOwnAdvertisedDepth, isEmergency, profileImageNumber, isCallable)
                        }
                    }.also {
        ...
        try {
            if (ContextCompat.checkSelfPermission(activity, Manifest.permission.BLUETOOTH_ADVERTISE) == PackageManager.PERMISSION_GRANTED) {
                Log.i(TAG, "광고 시작 시도: sID=$serverUserId, Nick='$processedNickname', MyDepth=$currentOwnAdvertisedDepth, Callable=$isCallable")
    ...
    private fun buildLanternPacket(
        serverUserId: Long,
        processedNickname: String,
        currentOwnAdvertisedDepth: Int,
        isEmergency: Boolean,
        ...
        try {
            ...
            var flags: Byte = 0
            if (isEmergency) {
                flags = (flags.toInt() or EMERGENCY_FLAG_BIT.toInt()).toByte()
            }
            if (isCallable) {
                flags = (flags.toInt() or CALLABLE_FLAG_BIT.toInt()).toByte()
            }

            // 패킷 구성: (1)데이터타입 + (1)버전 + (4)서버ID + (1)Depth + (1)닉네임길이 + (n)닉네임 + (1)플래그 + (1)프로필이미지번호
            val packetSize = 1 + 1 + BleConstants.SERVER_USER_ID_BYTES + BleConstants.DEPTH_BYTES + 1 + nicknameBytes.size + 1 + 1
            if (packetSize > BleConstants.MAX_ADV_DATA_SIZE) { // 상수 사용
                Log.e(TAG, "최종 패킷 크기($packetSize)가 BLE 광고 제한(${BleConstants.MAX_ADV_DATA_SIZE}바이트)을 초과합니다.")
            ...
            if (Log.isLoggable(TAG, Log.DEBUG)) {
                Log.d(TAG, "패킷 구성: " +
                      ...
                      "nameLen=${nicknameBytes.size}, " +
                        ...
                        "flags=$flags (Emergency=${(flags.toInt() and EMERGENCY_FLAG_BIT.toInt()) != 0}, Callable=${(flags.toInt() and CALLABLE_FLAG_BIT.toInt()) != 0}), " +
